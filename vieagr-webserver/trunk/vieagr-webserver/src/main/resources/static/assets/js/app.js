...
$(function() {
  ...
  function addIframeTal(iframeUrl,tabName,iframeUuid){
    ...
    var content = '<div class="am-tab-panel" style="height: 100%"><iframe id="'+iframeUuid+'" src="'+iframeUrl+'" class="admin-iframe-html" style="width: 100%;height: 100%;"></iframe></div>';
    $nav.append(nav);
    $bd.append(content);
    $tab.tabs('refresh');
  ...
  for (var i = 1; i < TAB_JSON.list.length; i++) {
    if (TAB_JSON.list[i].isHtml == true) {
      html = html + '<li><a href="javascript:void(0);" iframe-url="' + TAB_JSON.list[i].listUrl + '" ' +
          'iframe-name="' + TAB_JSON.list[i].listName + '" iframe-uuid="' + TAB_JSON.list[i].listUuid + '" class="admin-open-new-tab">' +
