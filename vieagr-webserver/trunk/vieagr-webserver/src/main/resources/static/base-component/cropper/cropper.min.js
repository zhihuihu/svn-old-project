...
/*$('.am-cj-yt-img > img').cropper({
    aspectRatio: 1 / 1,
    preview: '.am-cj-after-body',
    crop: function(data) {
    }
});*/
/**
 ...
 * @returns {string}
 */
function $getCropperHtml(id){
    var cropperHtml = '<div class="am-cj-body"> ' +
        '<div class="am-cj-yt-body"> ' +
        '<div class="am-cj-yt-body-left"> ' +
        '<div class="am-cj-yt-img"> ' +
        '<img src="../assets/i/head-icon.png"> ' +
        '</div> ' +
        '<div class="am-form-group am-form-file"> ' +
        '<div> ' +
        '<button type="button" class="am-btn am-btn-danger am-btn-sm am-margin-top-sm am-cj-btn-choose"> ' +
        '<i class="am-icon-cloud-upload"></i> 选择要裁剪的图片 ' +
        '</button> ' +
        '</div> ' +
        '<input type="file" id="am-cj-input-img"> ' +
        '</div> ' +
        '<button type="button" class="am-btn am-btn-primary am-btn-sm am-cj-after-body-img-save am-cj-sure"> ' +
        '<i class=""></i>确定 ' +
        '</button> ' +
        '</div> ' +
        '</div> ' +
        '<div class="am-cj-after-body"> ' +
        '<img class="am-cj-after-body-img" src="../assets/i/head-icon.png"> ' +
        '</div> ' +
        '<div style="clear: both"></div> ' +
        '</div>';
    var finalHtml = '<div id="'+id+'" style="position: relative;">' + cropperHtml + '</div>'
    return finalHtml
};

function $getCropper34Html(id){
    var cropperHtml = '<div class="am-cj-body"> ' +
        '<div class="am-cj-yt-body"> ' +
        '<div class="am-cj-yt-body-left"> ' +
        '<div class="am-cj-yt-img"> ' +
        ...
        '</div> ' +
        '</div> ' +
        '<div class="am-cj-after-body" id="am-cj-after-body"> ' +
        '<img class="am-cj-after-body-img" src="../assets/i/head-icon.png"> ' +
        '</div> ' +
        '<div style="clear: both"></div> ' +
        '</div>';
 ...
 * 裁剪图片的尺寸
 */
var $cropperImageSize = {width:180,height:180};//{width:280,height:160}

var $cropperSize = '1 / 1';//7:4
/**
 * 初始化裁剪方法
 * @param id
 */
function $initCropper(cropperAppendInput,headImagePopPage,cropperImageSize,cropperSize){
	$cropperAppendInput = cropperAppendInput;
	$headImagePopPage = headImagePopPage;
	if(cropperImageSize){
		$cropperImageSize = cropperImageSize;
	}
	if(cropperSize){
		$cropperSize = cropperSize;
		$('#am-cj-after-body').removeClass('am-cj-after-body');
		$('#am-cj-after-body').addClass('am-cj-after-body34');
	}
	$('.am-cj-yt-img > img').cropper({
	    aspectRatio: $cropperSize,
        preview: '.am-cj-after-body',
	    crop: function(data) {
	    }
	});
	$('.am-cj-body').css('min-height','320px');
	$('.am-cj-body').css('max-height','320px');
};

/**
 * 选择图片
 */
$('body').on('change','#am-cj-input-img',function(e){
    $('.am-cj-yt-img > img').cropper('replace',getObjectURL($(this)[0].files[0]));
});

$('body').on('click','.am-cj-after-body-img-save',function(e){
	$($cropperAppendInput).attr('src',$('.am-cj-yt-img > img').cropper('getCroppedCanvas',$cropperImageSize).toDataURL("image/jpeg"));
	if($headImagePopPage){
		layer.close($headImagePopPage);
	}
});
